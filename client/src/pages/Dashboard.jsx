...
export default function Dashboard() {
  ...
  const toggleFavorite = async (amenityName) => {
    ...
    try {
      if (favorites.includes(amenityName)) {
        // Remove favorite
        const res = await axios.delete(`${BASE_URL}/api/auth/favorites`, {
          headers: { Authorization: `Bearer ${token}` },
          data: { amenity: amenityName }
        });
        setFavorites(res.data.favorites);
        ...
        // Add favorite
        const res = await axios.post(`${BASE_URL}/api/auth/favorites`, { amenity: amenityName }, {
          headers: { Authorization: `Bearer ${token}` }
        });
        setFavorites(res.data.favorites);
      }
