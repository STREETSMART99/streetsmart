...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      console.log("Extracted isAdmin status in frontend:", isAdmin);
      if (status === 200) {
        localStorage.setItem("token", token);
        Swal.fire({ title: message, icon: "success", draggable: true });
  ...
  return (
    <div className="login-page w-full min-h-screen flex items-center justify-center bg-gray-100 p-10 relative">
      <div className="login-top-title">Street Smart</div>
      <div className="shape top-left"></div>
      <div className="shape bottom-left"></div>

      <div className="lg:container mx-auto flex items-center gap-16 justify-between w-full">
        {/* Left Section */}
        <div className="left_wrapper">
          <img src="src/assets/logolog.png" alt="Login" />
        </div>
        {/* Right Section */}
        <div className="right_wrapper space-y-4 max-w-[640px] w-full h-auto">
          <h3 className="text-5xl text-[#313131] font-semibold capitalize">Welcome!</h3>
